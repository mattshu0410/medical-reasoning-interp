...
7. diagnostic_commitment: Committing to a specific leading diagnosis. 
8. final_answer_emission: Formatting the final diagnosis into the desired answer format, i.e. "Now, for the output, I need to provide internal reasoning within <think> tags and the final diagnosis within <answer> tags."
9. other: Use only if the sentence does not fit any of the above tags or is purely stylistic. 
...
def call_deepseek(client: Optional[OpenAI], case_prompt: str, reasoning_trace: str, model: str = "deepseek-chat", retries: int = 3, sleep: float = 2.0, jitter: float = 0.25) -> Optional[Dict[str, Any]]:
...
populatePmcidOptions();
    ...
    def process_row(idx: int, row: pd.Series) -> Optional[Dict[str, Any]]:
        ...
        obj = call_deepseek(None, str(row["case_prompt"]), str(row["reasoning_trace"]), model=model, retries=retries, sleep=retry_sleep, jitter=retry_jitter)
        if obj is None:
            return None
        # labels = normalize_labels(obj)
        # try:
...
if __name__ == "__main__":
    ...
    p.add_argument("--workers", type=int, default=4, help="Number of parallel workers (1 for sequential)")
